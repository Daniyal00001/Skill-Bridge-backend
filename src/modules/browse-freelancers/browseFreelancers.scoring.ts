...
// // ── 1. SKILL MATCH (35%) ─────────────────────────────────────────
// // How many of client's required skills does freelancer have?
// function calcSkillMatch(
//   freelancerSkills: string[],
//   clientRequiredSkills: string[],
// ): number {
//   if (!clientRequiredSkills.length) return 60; // no requirements = neutral
...
// // ── 2. RATING SCORE (25%) ────────────────────────────────────────
// // Based on averageRating (0-5) + totalReviews volume bonus.
// // WHY volume bonus: 5.0 with 1 review < 4.8 with 50 reviews (reliability)
// function calcRatingScore(
//   averageRating?: number | null,
//   totalReviews?: number,
// ): number {

// no reviews = neutral (new freelancer)
//   if (!averageRating) return 40;

//   // Base: rating scaled to 0-90
//   const base = (Math.min(averageRating, 5) / 5) * 90;

//   // Volume bonus: up to 10 extra points for 20+ reviews
...
// // AVAILABLE = full points. BUSY = partial. UNAVAILABLE = 0.
...
//       return 100;
//     case "BUSY":
//       return 40; // can still take work but limited
//     case "UNAVAILABLE":
//       return 0;
//     default:
//       return 0;
...
//   if (freelancerHourlyRate >= min && freelancerHourlyRate <= max) return 100;

//   // Too expensive
//   if (freelancerHourlyRate > max) {
//     const overage = freelancerHourlyRate - max;
//     const score = Math.max(0, 100 - (overage / max) * 100);
//     return Math.round(score);
//   }

//   // Too cheap (unusual but possible — client wants experienced talent)

//   const daysSinceLogin =
//     (Date.now() - new Date(lastLoginAt).getTime()) / 86_400_000;

//   // 0 days = 100, decays to 0 over 30 days
//   return Math.round(Math.max(0, 100 - (daysSinceLogin / 30) * 100));
// }

// // ── 7. PERSONAL BOOST (multiplier) ──────────────────────────────
// // Boost if client has shown interest in similar profiles.
...
//   // Already viewed = slight de-boost (prevent repetition)
//   if (client.viewedFreelancerIds.includes(freelancer.id)) {
//     boost -= 0.05;
//   }

//   // Previously hired → strong negative (already worked together — show new talent)
...
/**
 * browseFreelancers.scoring.ts
 * location: backend/src/modules/browse-freelancers/browseFreelancers.scoring.ts
 *
 * Pure scoring math — no DB calls, no side effects.
